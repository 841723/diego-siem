...
export default function RulesPage() {
    ...
    async function handleDelete(rule: Rule) {
        try {
            ...
            setActionError((err as Error).message || "Error eliminando regla");
        } finally {
            setConfirmRule(null);
        }
    }
    ...
    const rows = rules.map((rule) => [
        ...
        <span className={`rounded px-2 py-0.5 text-xs ${rule.enabled ? "bg-success/20 text-success" : "bg-warning/20 text-warning"}`}>
            {rule.enabled ? "Enabled" : "Disabled"}
        </span>,
        <span className="text-xs text-muted">{rule.type}</span>,
        <span className="text-xs text-muted">{rule.severity}</span>,
        <span className="text-xs text-muted">{formatDate(rule.last_execution_at)}</span>,
        ...
        <main className="flex h-full flex-col overflow-hidden">
            <div className="flex shrink-0 items-center justify-between border-b border-border bg-background px-6 py-4">
                <div>
                    ...
                    <p className="text-xs text-muted">Gestiona reglas de detección y monitorea alertas generadas</p>
                </div>
            ...
            <ConfirmModal
                ...
                message={`¿Seguro que quieres eliminar "${confirmRule?.name}"?`}
